  ...
  try {
    ...
    if (message?.order?.quote) {
      validateOrderQuote(message, result, {
        ...
        validateTotalMatch: false, // FIS12 total check is handled by validateFIS12LoanQuote (excludes NET_DISBURSED_AMOUNT)
        validateItemPriceConsistency: false,
        flowId,
      });
      // FIS12 loan-specific: required titles, total sum, NET_DISBURSED_AMOUNT formula
