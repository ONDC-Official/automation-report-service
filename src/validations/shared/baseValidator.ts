...
export function createDomainValidator(
  actionToFileMap: Record<ValidationAction, string | undefined>,
  ...
  return async function validate(
    ...
    try {
      ...
      try {
        const fileName = actionToFileMap[action];
        if (!fileName || !version) {
          testResults.failed.push(`Incorrect version or unsupported action: ${action}`);
          return testResults;
        }
