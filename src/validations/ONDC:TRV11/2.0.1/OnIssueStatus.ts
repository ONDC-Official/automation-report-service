...
export default async function on_issue_status(
  ...
  // IGM 1.0.0 has 'issue_actions', IGM 2.0.0 has 'update_target' and 'level'
  const isIgm1 = issue?.issue_actions !== undefined || 
                 (message?.update_target === undefined && issue?.level === undefined);
  
  let result: TestResult;
  
  if (isIgm1) {
    // IGM 1.0.0
    result = await DomainValidators.igm1OnIssueStatus(element, sessionID, flowId, actionId);
    if (message && result.passed.length === 0 && result.failed.length === 0) {
      validateIgm1OnIssueStatus(message, result);
    }
  } else {
    // IGM 2.0.0 - use on_issue_status with update_target
    result = { response: {}, passed: [], failed: [] };
